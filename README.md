...
当前项目**没有发布可直接拉取的应用镜像**，不要在 Docker Hub 搜索并下载同名的第三方镜像。仓库中的 `compose.yaml` 会使用 `Dockerfile` 在飞牛本机完成构建：

- 第一次构建自动下载官方基础镜像 `python:3.12-slim`。
- 随后安装 `requirements.txt` 中的依赖并复制 Solis_Timelapse 源码。
- 最终生成本地镜像 `solis_timelapse:latest`。
- 运行的容器和服务名都是 `solis_timelapse`。

因此首次部署必须把完整项目源码放到飞牛上。以后更新源码后重新构建即可，不需要手动拉取某个 Solis_Timelapse 镜像。
...
解压后把完整源码目录上传到 `/vol1/1000/solis_timelapse/app`。需要保留 `compose.yaml`、`Dockerfile`、`requirements.txt`、`src`、`webui`、`config` 和 `docker`，不能只上传 Compose 文件。

推荐目录结构如下：

```text
/vol1/1000/solis_timelapse/
  app/                    # 项目源码，Compose 项目目录
    compose.yaml
    Dockerfile
    requirements.txt
    .env.example
...
`app` 可以在更新时整体替换；另外四个数据目录不要放进 `app`，也不要在更新源码时删除。
...
- `image: solis_timelapse`：给本机构建结果命名，不会从远程下载同名应用镜像。
- `build: .`：使用 `app` 目录中的 Dockerfile 和源码构建镜像，所以 Compose 项目目录必须是 `app`。
...
6. 点击“构建并启动”“部署”或“确定”。第一次会下载 `python:3.12-slim`、安装 Python 依赖并构建本地镜像，耗时取决于网络和飞牛 CPU。
...
1. 先备份 `.env`；不要删除同级四个数据目录。
2. 下载新版本源码，替换 `app` 中的代码，保留或恢复 `.env`。
3. 在飞牛 Compose 页面点击“重新构建/重新部署”，或通过 SSH 执行：
...
仅执行 `docker compose restart` 不会应用新源码或 Compose 配置；更新后必须重新构建。

### 10. 常见问题

**构建时一直下载或速度很慢**

首次构建需要从 Docker Hub 下载 `python:3.12-slim`，还要从 Python 软件源安装依赖。先确认飞牛能访问这些站点；镜像加速器属于飞牛 Docker 的系统配置，不要把不可信镜像地址写进本项目 Compose。
