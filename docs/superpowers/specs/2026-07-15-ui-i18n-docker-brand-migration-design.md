...
- 不支持从 Docker 容器浏览 `/media` 以外的宿主机目录。
...
- 飞牛默认数据目录示例：`/vol1/1000/docker/solis_timelapse`
...
```env
MEDIA_PATH=/vol1/1000/照片/延时摄影
DATA_PATH=/vol1/1000/docker/solis_timelapse
PUID=1000
PGID=1000
```

`/vol1/1000` 仅是常见首位管理员示例，不写死到程序。用户应在飞牛文件管理器中通过“详细信息 → 复制原始路径”取得真实路径，或通过 SSH 使用 `id 用户名` 确认 PUID/PGID。

### 7.3 容器目录

- `/media`：原始照片，只读。
- `/data/workspace`：当前任务和 JPEG 结果。
- `/data/output`：当前 MP4。
- `/data/archive`：归档历史。
- `/data/config/local.yaml`：容器本地持久化设置。

Docker 启动时必须验证 `/media` 可读、`/data` 可写。失败时启动日志给出明确原因，不能回退写入容器临时层。

### 7.4 目录选择

- Windows 本地模式保留原生文件夹选择窗口。
- Docker 模式使用 WebUI 目录浏览器。
- Docker 目录浏览 API 只允许列出 `/media` 及其子目录。
...
5. Docker 的 `/media:ro` 在操作系统层进一步限制误写。

## 9. 错误处理

- 无效主题或语言设置自动回退，不阻止页面启动。
- 翻译缺失使用中文兜底，并保留可诊断日志。
- Docker 未挂载 `/media`、`/data` 不可写或权限不匹配时，健康检查失败并输出具体路径和权限问题。
