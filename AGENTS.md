...
本文件适用于整个仓库。修改前先阅读 `README.md`、被 Git 忽略的 `DEVLOG.md`、`PLAYBOOK.md` 和相关测试，并区分已跟踪源码、外部素材与本机运行数据。
...
- `DEVLOG.md` 与 `PLAYBOOK.md` 继续仅在本机维护并由 Git 忽略；`README.md` 只保留用户可见的项目介绍、安装、使用方法和必要限制。
...
文档或迁移边界变更至少检查最终 diff、`git diff --check`、相关 Markdown 路径、`git check-ignore -v` 和 `git status`，并确认本地维护文件、运行数据、敏感配置与真实媒体均未进入暂存区。
